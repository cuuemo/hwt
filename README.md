...
工作站客户端运行时会把日志写成加密文件 `.log.enc`（RSA+AES-GCM 封装），避免明文落盘。出问题时让用户把文件发给管理员，然后：

1. 云端管理后台侧边栏点击「客户端日志 / Client Logs」
2. 把 `.log.enc` 文件拖进上传框（单文件 ≤ 20 MB）
3. 页面直接显示解密后的明文行，支持搜索过滤和下载为 `.txt`

解密在云端内存中完成，明文不落盘、不入库；RSA 私钥仅存在于云端容器。无需再用 `cloud/backend/tools/decrypt_log.py` 命令行脚本（脚本仍然保留）。
