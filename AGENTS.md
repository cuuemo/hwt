...
Use 4-space indentation across Rust, Python, and TypeScript/Vue files. Follow Rust 2021 defaults: `snake_case` for functions/modules, `CamelCase` for types, and keep crate names aligned with existing `hwt-*` patterns. In Python, keep modules lowercase and test helpers in `conftest.py`. In Vue, use `PascalCase.vue` for views and layouts, and keep API helpers under `src/api/`.
