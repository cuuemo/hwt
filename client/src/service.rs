            ...
            loop {
                match crate::protocol::run_cleanup_cycle(state.clone()).await {
                    Ok(server_ip) => {
                        ...
                        // STOP — do NOT uninstall. The SYSTEM process exits so it
                        // is not resident while games run, but the service stays
                        // REGISTERED. That is what lets it survive in the captured
                        // master image and auto-start on every boot of the
                        // deployed restore-on-reboot machines. (Self-deleting here
                        // would erase the service from the image before packaging.)
                        log::info!(
                            "Heartbeat handed off; stopping service (stays registered for next boot)"
                        );
                        break;
                    }
                    Err(e) => {
