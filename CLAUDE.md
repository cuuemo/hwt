...
HWT (网维系统) is an internet cafe management system with three components:
...
cargo build --release --target x86_64-pc-windows-gnu -p hwt-server  # cross-compile for Windows
...
- Rust: `snake_case` functions, `CamelCase` types, crate names follow `hwt-*` pattern
